...
from TheoreticallyOptimalStrategy import testPolicy as testPolicy_tos
from strategy_evaluation.indicators import *
from util import get_data


def testPolicy(symbols="AAPL", sd='2010-01-01', ed='2011-12-31', sv=100000):

    start_date = sd
    end_date = ed

    port_val = get_data([symbols], dates=pd.date_range(start_date, end_date))
    ...
    williamsR = calculate_williamsR(port_val.copy(), window=4, plot=False, ret_val=True, symbol=symbols)[['williams']]
    rsi = calculate_RSI_EMV(port_val.copy(), window=4, plot=False, ret_val=True, symbol=symbols)[['RSI_EMV']]
    momentum = calculate_momentum(port_val.copy(), window=8, plot=False, ret_val=True, symbol=symbols)[['momentum']]
...
if __name__ == '__main__':
    ...
    port_val_tos = compute_portvals(orders=portval_tos, start_val=100000, commission=9.95, impact=0.00)
    ...
    port_val_tos_norm = port_val_tos / port_val_tos.iloc[0]
    ...
    port_val_tos_norm.rename(columns={'portval': 'TOS'}, inplace=True)

    colors = {'Benchmark': 'green', 'Manual Strategy': 'red', 'TOS': 'blue'}
    ax = pd.concat([port_vals_bchm_norm, port_vals_norm], axis=1).plot(title="Normalized Benchmark with Manual Strategy and TOS", fontsize=12, color=colors)
    ax.set_xlabel("Date")
    ax.set_ylabel("Normalized Portfolio")
    plt.grid(True, 'both')
    plt.savefig('MS.jpg', dpi=550)
